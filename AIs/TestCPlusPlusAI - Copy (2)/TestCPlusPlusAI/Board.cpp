...
{ 
  ...
  for(y = 0; y < BOARD_SIZE; y++)
    ...
    {
      ctor_Piece(&m->board[x][y]);
      ctor_Loc(&m->board[x][y].location, x,y);     
...
void ctor_Board(Board* m, Board* other )
{
  ...
  for(y = 0; y < BOARD_SIZE; y++)
  {
    for(x = 0; x < BOARD_SIZE; x++)
    {
      ...
      //location
      ctor_Loc(&curPiece.location, x, y);
  ...
  for(y = 0; y < BOARD_SIZE; y++)
  {
    for(x = 0; x < BOARD_SIZE; x++)
    {
      ...
      for(Piece** pieceInGroup = FastSetPiece_begin(&otherPiece->ownedLibertyGroup.group);
      ...
      for(Piece** pieceInGroup = FastSetPiece_begin(&otherPiece->ownedLibertyGroup.liberties);
...
{  
  ...
  Piece* sqrSur[4];
  int cSqrSur = SurroundingSquares(sqrSur, location, m);    
