...
namespace MainAI
{
    class Program
    {
        ...
        //Make this considerably smaller, while having roughly the same speed
        //by making a good C# preprocessor

        //Regular call structure (roughly):
        //GetAndPlayMove
        //  GetBest
        ...
        public class TestDotNetGoPlayer : SimpleGoPlayer
        {
            ...
            public static int monteCarloCount = 5;
            ...
            {
                get { return "DotNetGo With Preprocessor"; }
            }
        ...
        /// <summary>
        /// I am fairly sure this can and should be fully const after its creation,
        /// all of its functions are just helpers for it.
        /// </summary>
        public class Piece
        {
            enum Pieces { Empty, Black, White };

            Pieces curPiece = Pieces.Empty;

            public bool Black
            {
                get
                {
                    return curPiece == Pieces.Black;
                }
            }
            ...
            public Piece OppositePiece
            {
                get
                {
                    if (curPiece == Pieces.Black)
                    {
                        return new Piece(Pieces.White);
                    }
                    else if (curPiece == Pieces.White)
                    {
                        return new Piece(Pieces.Black);
                    }
                    else
                    {
                        throw new Exception("You cannot get the opposite of an empty square, you are doing something wrong.");
                    }
                }
            }
        }

        public class NewDotNetGoPlayer : SimpleGoPlayer
        {
            static double monteCarloPassAmount = 0;
            static double monteCarloNotPassAmount = 0;
            ...
            public Board board = new Board(boardSize, boardSize);


            public static double komi = 0; //SHOULD NOT BE STATIC


            public static void MetaPlayPiece(Piece curPiece, Board curBoard, int[,] libertyGroups, int[,] groupNumbers, ref int curGroupCount, int x, int y, ref int blackCaptured, ref int whiteCaptured)
            {                                                                
